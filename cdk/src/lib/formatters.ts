...
export const formatSharedFilesDbResponse = (
...
export const formatOwnedFilesDbResponse = (
  response: QueryCommandOutput,
): OwnedFile[] => {
  if (!response.Items) {
    return [];
...
export const formatCognitoUserResponse = (
  ...
  const attributes = Object.fromEntries(
    cognitoResponse.UserAttributes.map(({ Name, Value }) => [Name, Value]),
  );
...
export const formatS3MetaDataResponse = (
  s3Response: HeadObjectCommandOutput,
): FileMetaData => {
  if (
