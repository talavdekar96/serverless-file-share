...
if (!process.env.USER_TABLE_NAME) {
  throw new Error("User table environment variable missing");
}

if (!process.env.ROLE_TABLE_NAME) {
  throw new Error("Role table environment variable missing");
}
...
export const ROLE_TABLE_NAME = process.env.ROLE_TABLE_NAME;
