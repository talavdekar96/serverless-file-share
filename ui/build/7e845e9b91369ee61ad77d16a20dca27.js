ace.define(
  ...
  function (e, t, n) {
            ...
            { token: "punctuation.separator.key-value", regex: "=>" },
            {
              ...
              onMatch: function (e, t, n) {
                var r =
                  ...
                  a = e.split(this.splitRegex);
                return (
                  n.push(r, a[3]),
                  [
                    { type: "constant", value: a[1] },
                    { type: "string", value: a[2] },
                    { type: "support.class", value: a[3] },
                    { type: "string", value: a[4] },
                  ]
                );
              },
              regex: "(<<[-~]?)(['\"`]?)([\\w]+)(['\"`]?)",
    ...
    r.inherits(_, a), (t.RubyHighlightRules = _);
  }
),
  ace.define(
      ...
      (function () {
        (this.checkOutdent = function (e, t) {
          ...
          (this.autoOutdent = function (e, t) {
            ...
            if (!n) return 0;
            var a = n[1].length,
              s = e.findMatchingBracket({ row: t, column: a });
            if (!s || s.row == t) return 0;
            var i = this.$getIndent(e.getLine(s.row));
            e.replace(new r(t, 0, t, a - 1), i);
          }),
          (this.$getIndent = function (e) {
            return e.match(/^\s*/)[0];
          });
      }).call(a.prototype),
        (t.MatchingBraceOutdent = a);
    }
  ),
  ace.define(
        ...
        function () {
          (this.indentKeywords = {
            ...
            (this.getFoldWidget = function (e, t, n) {
              var r,
                a = e.getLine(n),
                s = this.foldingStartMarker.test(a),
                i = this.foldingStopMarker.test(a);
              if (s && !i)
                if ((r = a.match(this.foldingStartMarker))[1]) {
                  if (
                    "if" == r[1] ||
                    "else" == r[1] ||
                    "while" == r[1] ||
                    ...
                    if ("else" == r[1] && !1 === /^\s*else\s*$/.test(a)) return;
                    if (!1 === /^\s*(?:if|else|while|until|unless)\s*/.test(a))
                      return;
                  }
                  if ("when" == r[1] && !0 === /\sthen\s/.test(a)) return;
              ...
              if ("markbeginend" != t || !i || (s && i)) return "";
              if ("end" === (r = a.match(this.foldingStopMarker))[3]) {
            ...
            (this.getFoldWidgetRange = function (e, t, n) {
              var r,
                a = e.doc.getLine(n);
              return (r = this.foldingStartMarker.exec(a))
                ? r[1] || r[3]
                  ? this.rubyBlock(e, n, r.index + 2)
                  : this.openingBracketBlock(e, "{", n, r.index)
                : (r = this.foldingStopMarker.exec(a))
                ? ("end" === r[3] &&
            ...
            (this.rubyBlock = function (e, t, n, r) {
              var a = new i(e, t, n),
                o = a.getCurrentToken();
              if (o && ("keyword" == o.type || "comment.multiline" == o.type)) {
                var l = o.value,
                  c = e.getLine(t);
                switch (o.value) {
                  case "if":
                  ...
                  case "while":
                  case "until":
                    if (!new RegExp("^\\s*" + o.value).test(c)) return;
                  ...
                  case "ensure":
                    ...
                    break;
                  case "else":
                    if (!new RegExp("^\\s*" + o.value + "\\s*$").test(c))
                      return;
                    u = 1;
                    break;
                  default:
                ...
                if (u) {
                  var h =
                    ...
                    _ = [];
                  if (
                    (_.push(a.getCurrentTokenRange()),
                    (a.step = -1 === u ? a.stepBackward : a.stepForward),
                    "comment.multiline" == o.type)
                  ) {
                    for (; (o = a.step()); )
                      if ("comment.multiline" === o.type)
                        if (1 == u) {
                          if (((h = 6), "=end" == o.value)) break;
                        } else if ("=begin" == o.value) break;
                  } else
                    for (; (o = a.step()); ) {
                      var f = !1;
                      if ("keyword" === o.type) {
                        var p = u * this.indentKeywords[o.value];
                        switch (
                          ((c = e.getLine(a.getCurrentTokenRow())), o.value)
                        ) {
                          case "do":
                            for (var k = a.$tokenIndex - 1; k >= 0; k--) {
                              var x = a.$rowTokens[k];
                              if (
                                x &&
                                ("while" == x.value ||
                        ...
                        if (p > 0) g.unshift(o.value);
                          ...
                          if ((g.shift(), !g.length)) {
                            if (
                              ...
                              break;
                            if ("do" == o.value && -1 == u && 0 != p) break;
                            if ("do" != o.value) break;
                          }
                          0 === p && g.unshift(o.value);
                        }
                      }
                    }
                  if (!o) return null;
                  if (r) return _.push(a.getCurrentTokenRange()), _;
                  t = a.getCurrentTokenRow();
                  if (-1 === u) {
                    if ("comment.multiline" === o.type) var m = 6;
                    else m = e.getLine(t).length;
                    return new s(t, m, d - 1, h);
                  }
                  return new s(d, h, t - 1, e.getLine(t - 1).length);
                }
              }
            });
        }.call(o.prototype);
          ...
          (this.lineCommentStart = "#"),
            (this.getNextLineIndent = function (e, t, n) {
              var r = this.$getIndent(t),
                a = this.getTokenizer().getLineTokens(t, e).tokens;
            ...
            (this.autoOutdent = function (e, t, n) {
              ...
              var a = this.$getIndent(r),
                s = t.getLine(n - 1),
                i = this.$getIndent(s),
                l = t.getTabString();
              i.length <= a.length &&
                a.slice(-l.length) == l &&
                t.remove(new o(n, a.length - l.length, n, a.length));
            }),
            (this.getMatching = function (e, t, n) {
              ...
              var a = e.getTokenAt(t, n);
              if (a && a.value in this.indentKeywords)
  ...
  ace.require(["ace/mode/ruby"], function (e) {
