ace.define(
  ...
  ace.define(
    ...
    function (e, t, n) {
      ...
      o.inherits(p, r), (t.CssHighlightRules = p);
    }
  ),
  ace.define(
            ...
            this.add("colon", "insertion", function (e, t, n, o, r) {
              if (":" === r && n.selection.isEmpty()) {
                ...
                if (
                  ...
                  var u = o.doc.getLine(i.row);
                  if (":" === u.substring(i.column, i.column + 1))
                    return { text: "", selection: [1, 1] };
                  if (/^(\s+[^;]|\s*$)/.test(u.substring(i.column)))
                    return { text: ":;", selection: [1, 1] };
                }
              }
            }),
            this.add("colon", "deletion", function (e, t, n, o, r) {
              ...
              if (!r.isMultiLine() && ":" === i) {
                var s = n.getCursorPosition(),
                  ...
                  u = l.getCurrentToken();
                if (
                  (u && u.value.match(/\s+/) && (u = l.stepBackward()),
                  u && "support.type" === u.type)
                )
                  if (
                    ";" ===
                    o.doc
        ...
        this.add("string_dquotes", "insertion", function (e, t, n, o, r) {
          if ('"' == r || "'" == r) {
            ...
            var u = n.getCursorPosition(),
              c = o.doc.getLine(u.row).substring(u.column, u.column + 1),
              d = new a(o, u.row, u.column),
              g = d.getCurrentToken();
            if (c == s && (i(g, "attribute-value") || i(g, "string")))
              return { text: "", selection: [1, 1] };
            if ((g || (g = d.stepBackward()), !g)) return;
            for (; i(g, "tag-whitespace") || i(g, "whitespace"); )
              g = d.stepBackward();
            var p = !c || c.match(/\s/);
            if (
              (i(g, "attribute-equals") && (p || ">" == c)) ||
              (i(g, "decl-attribute-equals") && (p || "?" == c))
            )
          ...
          this.add("autoclosing", "insertion", function (e, t, n, o, r) {
            if (">" == r) {
              var s = n.getSelectionRange().start,
                l = new a(o, s.row, s.column),
                u = l.getCurrentToken() || l.stepBackward();
              if (
                !u ||
                !(
                  i(u, "tag-name") ||
                  i(u, "tag-whitespace") ||
                  i(u, "attribute-name") ||
                  i(u, "attribute-equals") ||
                  i(u, "attribute-value")
                )
              )
                return;
              if (i(u, "reference.attribute-value")) return;
              if (i(u, "attribute-value")) {
                var c = l.getCurrentTokenColumn() + u.value.length;
                if (s.column < c) return;
                if (s.column == c) {
                  var d = l.stepForward();
                  if (d && i(d, "attribute-value")) return;
                  l.stepBackward();
                }
              }
              if (/^\s*>/.test(o.getLine(s.row).slice(s.column))) return;
              for (; !i(u, "tag-name"); )
                if ("<" == (u = l.stepBackward()).value) {
                  u = l.stepForward();
                  break;
                }
              var g = l.getCurrentTokenRow(),
                p = l.getCurrentTokenColumn();
              if (i(l.stepBackward(), "end-tag-open")) return;
              var m = u.value;
              if (
                (g == s.row && (m = m.substring(0, s.column - p)),
          ...
          this.add("autoindent", "insertion", function (e, t, n, o, r) {
            if ("\n" == r) {
              var i = n.getCursorPosition(),
                ...
                u = l.getCurrentToken();
              if (u && -1 !== u.type.indexOf("tag-close")) {
                if ("/>" == u.value) return;
                for (; u && -1 === u.type.indexOf("tag-name"); )
                  u = l.stepBackward();
                if (!u) return;
                var c = u.value,
                  d = l.getCurrentTokenRow();
                if (!(u = l.stepBackward()) || -1 !== u.type.indexOf("end-tag"))
                  return;
                if (this.voidElements && !this.voidElements[c]) {
                  var g = o.getTokenAt(i.row, i.column + 1),
                    p = ((s = o.getLine(d)), this.$getIndent(s)),
                    m = p + o.getTabString();
                  return g && "</" === g.value
                    ? {
                        text: "\n" + m + "\n" + p,
      ...
      var o = e("../lib/oop"),
        ...
        h = function (e) {
          (this.fragmentContext = e && e.fragmentContext),
            ...
            (this.foldingRules = new c(this.voidElements, r.arrayToMap(m)));
        };
      o.inherits(h, a),
        function () {
          (this.blockComment = { start: "\x3c!--", end: "--\x3e" }),
            (this.voidElements = r.arrayToMap(p)),
            (this.getNextLineIndent = function (e, t, n) {
            ...
            (this.createWorker = function (e) {
              if (this.constructor == h) {
                var t = new g(["ace"], "ace/mode/html_worker", "Worker");
                return (
                  t.attachToDocument(e.getDocument()),
  ...
  ace.define(
    ...
    function (e, t, n) {
      ...
      o.inherits(a, r), (t.LuaHighlightRules = a);
    }
  ),
  ace.define(
      ...
      o.inherits(s, r),
        function () {
          (this.foldingStartMarker =
            ...
            (this.getFoldWidget = function (e, t, n) {
              var o = e.getLine(n),
                r = this.foldingStartMarker.test(o),
                a = this.foldingStopMarker.test(o);
              if (r && !a) {
                var i;
                if (
                  "then" == (i = o.match(this.foldingStartMarker))[1] &&
                  /\belseif\b/.test(o)
                )
                  return;
                if (i[1]) {
                  if ("keyword" === e.getTokenAt(n, i.index + 1).type)
                    return "start";
                } else {
                  if (!i[2]) return "start";
                  if (
                    "bracketedComment" ==
                      (s = e.bgTokenizer.getState(n) || "")[0] ||
                    "bracketedString" == s[0]
                  )
                    return "start";
                }
              }
              if ("markbeginend" != t || !a || (r && a)) return "";
              if ("end" === (i = o.match(this.foldingStopMarker))[0]) {
                if ("keyword" === e.getTokenAt(n, i.index + 1).type)
                  return "end";
              } else {
                if ("]" !== i[0][0]) return "end";
                ...
                if (
                  "bracketedComment" ==
                    (s = e.bgTokenizer.getState(n - 1) || "")[0] ||
                  "bracketedString" == s[0]
                )
                  return "end";
              }
            ...
            (this.getFoldWidgetRange = function (e, t, n) {
              var o,
                r = e.doc.getLine(n);
              return (o = this.foldingStartMarker.exec(r))
                ? o[1]
                  ? this.luaBlock(e, n, o.index + 1)
                  : o[2]
                  ? e.getCommentFoldRange(n, o.index + 1)
                  : this.openingBracketBlock(e, "{", n, o.index)
                : (o = this.foldingStopMarker.exec(r))
                ? "end" === o[0] &&
                  "keyword" === e.getTokenAt(n, o.index + 1).type
                  ? this.luaBlock(e, n, o.index + 1)
                  : "]" === o[0][0]
                  ? e.getCommentFoldRange(n, o.index + 1)
            ...
            (this.luaBlock = function (e, t, n, o) {
              var r = new i(e, t, n),
                s = {
                  function: 1,
                  do: 1,
              ...
              if (l && "keyword" == l.type) {
                ...
                if (d) {
                  ...
                  for (
                    ...
                    if ("keyword" === l.type) {
                      var m = d * s[l.value];
                      if (m > 0) c.unshift(l.value);
                      else if (m <= 0) {
                        if ((c.shift(), !c.length && "elseif" != l.value))
                          break;
                        0 === m && c.unshift(l.value);
                      }
                    }
                  if (!l) return null;
                  if (o) return r.getCurrentTokenRange();
                  t = r.getCurrentTokenRow();
                  return -1 === d
                    ? new a(t, e.getLine(t).length, p, g)
                    : new a(p, g, t, r.getCurrentTokenColumn());
                }
              }
            });
        }.call(s.prototype);
  ...
  ace.define(
    ...
    function (e, t, n) {
      ...
      var o = e("../lib/oop"),
        r = e("./text").Mode,
        a = e("./lua_highlight_rules").LuaHighlightRules,
        i = e("./folding/lua").FoldMode,
        s = e("../range").Range,
      ...
      o.inherits(u, r),
        function () {
          ...
          (this.getNextLineIndent = function (t, n, o) {
            var r = this.$getIndent(n),
              a = 0,
              i = this.getTokenizer().getLineTokens(n, t).tokens;
            return (
              "start" == t &&
                (a = (function (t) {
                  for (var n = 0, o = 0; o < t.length; o++) {
                    var r = t[o];
                    "keyword" == r.type
                      ? r.value in e && (n += e[r.value])
                      : "paren.lparen" == r.type
                      ? (n += r.value.length)
                      : "paren.rparen" == r.type && (n -= r.value.length);
                  }
              ...
              a > 0
                ...
                : a < 0 &&
                  ...
                  !this.checkOutdent(t, n, "\n")
                ? r.substr(0, r.length - o.length)
                : r
            );
          }),
            (this.checkOutdent = function (e, n, o) {
              if ("\n" != o && "\r" != o && "\r\n" != o) return !1;
              if (n.match(/^\s*[\)\}\]]$/)) return !0;
              var r = this.getTokenizer().getLineTokens(n.trim(), e).tokens;
              return (
                !(!r || !r.length) &&
                "keyword" == r[0].type &&
                -1 != t.indexOf(r[0].value)
              );
            }),
            (this.getMatching = function (t, n, o) {
              if (void 0 == n) {
                var r = t.selection.lead;
                (o = r.column), (n = r.row);
              }
              var a = t.getTokenAt(n, o);
              if (a && a.value in e)
                return this.foldingRules.luaBlock(t, n, o, !0);
            }),
            (this.autoOutdent = function (e, t, n) {
              var o = t.getLine(n).match(/^\s*/)[0].length;
              if (o && n) {
                var r = this.getMatching(t, n, o + 1);
                if (r && r.start.row != n) {
                  var a = this.$getIndent(t.getLine(r.start.row));
                  a.length != o &&
                    (t.replace(new s(n, 0, n, o), a),
                    t.outdentRows(new s(n + 1, 0, n + 1, 0)));
                }
              }
            }),
            (this.createWorker = function (e) {
  ...
  ace.define(
    ...
    function (e, t, n) {
      "use strict";
      var o = e("../lib/oop"),
        r = e("./html_highlight_rules").HtmlHighlightRules,
        a = e("./lua_highlight_rules").LuaHighlightRules,
        i = function () {
          r.call(this);
          var e = [
      ...
      o.inherits(i, r), (t.LuaPageHighlightRules = i);
    }
  ),
  ace.define(
    "ace/mode/luapage",
    ...
    function (e, t, n) {
      ...
      var o = e("../lib/oop"),
        r = e("./html").Mode,
        a = e("./lua").Mode,
        i = e("./luapage_highlight_rules").LuaPageHighlightRules,
        s = function () {
          r.call(this),
            (this.HighlightRules = i),
            this.createModeDelegates({ "lua-": a });
        };
      o.inherits(s, r),
        function () {
  ...
  ace.require(["ace/mode/luapage"], function (e) {
