ace.define(
  ...
  function (_, e, l) {
    "use strict";
    var E = _("../lib/oop"),
      A = _("./text_highlight_rules").TextHighlightRules;
    function T() {
      var _ = this.createKeywordMapper(
        {
      ...
      (this.$rules = {
        start: [
          ...
          { token: _, regex: "\\b[a-zA-Z_][a-zA-Z0-9_]*\\b" },
          {
    ...
    E.inherits(T, A), (e.LSLHighlightRules = T);
  }
),
  ace.define(
    ...
    function (_, e, l) {
      ...
      var E = _("../range").Range,
        A = function () {};
      (function () {
        (this.checkOutdent = function (_, e) {
          return !!/^\s+$/.test(_) && /^\s*\}/.test(e);
        }),
          (this.autoOutdent = function (_, e) {
            var l = _.getLine(e).match(/^(\s*\})/);
            if (!l) return 0;
            var A = l[1].length,
              T = _.findMatchingBracket({ row: e, column: A });
            if (!T || T.row == e) return 0;
            var R = this.$getIndent(_.getLine(T.row));
            _.replace(new E(e, 0, e, A - 1), R);
          }),
          (this.$getIndent = function (_) {
            return _.match(/^\s*/)[0];
          });
      }).call(A.prototype),
        (e.MatchingBraceOutdent = A);
    }
  ),
  ace.define(
    ...
    function (_, e, l) {
      ...
      var E = _("../../lib/oop"),
        A = _("../../range").Range,
        T = _("./fold_mode").FoldMode,
        R = (e.FoldMode = function (_) {
          _ &&
            ((this.foldingStartMarker = new RegExp(
              this.foldingStartMarker.source.replace(/\|[^|]*?$/, "|" + _.start)
            )),
            (this.foldingStopMarker = new RegExp(
              this.foldingStopMarker.source.replace(/\|[^|]*?$/, "|" + _.end)
            )));
        });
      E.inherits(R, T),
        function () {
          (this.foldingStartMarker = /([\{\[\(])[^\}\]\)]*$|^\s*(\/\*)/),
            ...
            (this.getFoldWidget = function (_, e, l) {
              var E = _.getLine(l);
              if (
                this.singleLineBlockCommentRe.test(E) &&
                !this.startRegionRe.test(E) &&
                !this.tripleStarBlockCommentRe.test(E)
              )
                return "";
              var A = this._getFoldWidgetBase(_, e, l);
              return !A && this.startRegionRe.test(E) ? "start" : A;
            }),
            (this.getFoldWidgetRange = function (_, e, l, E) {
              var A,
                T = _.getLine(l);
              if (this.startRegionRe.test(T))
                return this.getCommentRegionBlock(_, T, l);
              if ((A = T.match(this.foldingStartMarker))) {
                var R = A.index;
                if (A[1]) return this.openingBracketBlock(_, A[1], l, R);
                var t = _.getCommentFoldRange(l, R + A[0].length, 1);
                return (
                  t &&
                    !t.isMultiLine() &&
                    (E
                      ? (t = this.getSectionRange(_, l))
                      : "all" != e && (t = null)),
                  t
                );
              }
              if ("markbegin" !== e && (A = T.match(this.foldingStopMarker))) {
                R = A.index + A[0].length;
                return A[1]
                  ? this.closingBracketBlock(_, A[1], l, R)
                  : _.getCommentFoldRange(l, R, -1);
              }
            }),
            (this.getSectionRange = function (_, e) {
              for (
                var l = _.getLine(e),
                  E = l.search(/\S/),
                  T = e,
                ...
                var L = (l = _.getLine(e)).search(/\S/);
                if (-1 !== L) {
                  if (E > L) break;
                  var I = this.getFoldWidgetRange(_, "all", e);
                  if (I) {
                    if (I.start.row <= T) break;
                    if (I.isMultiLine()) e = I.end.row;
                    else if (E == L) break;
                  }
                  t = e;
                }
              }
              return new A(T, R, t, _.getLine(t).length);
            }),
            (this.getCommentRegionBlock = function (_, e, l) {
              for (
                var E = e.search(/\s*$/),
                  T = _.getLength(),
                  R = l,
              ...
              if (l > R) return new A(R, E, l, e.length);
            });
        }.call(R.prototype);
    }
  ),
  ace.define(
    ...
    function (_, e, l) {
      "use strict";
      var E = _("./lsl_highlight_rules").LSLHighlightRules,
        A = _("./matching_brace_outdent").MatchingBraceOutdent,
        T = _("./text").Mode,
        R = _("./folding/cstyle").FoldMode,
        t = function () {
      ...
      _("../lib/oop").inherits(t, T),
        function () {
          (this.lineCommentStart = ["//"]),
            (this.blockComment = { start: "/*", end: "*/" }),
            (this.getNextLineIndent = function (_, e, l) {
              var E = this.$getIndent(e),
                A = this.getTokenizer().getLineTokens(e, _),
                T = A.tokens;
              A.state;
              if (T.length && "comment.block.lsl" === T[T.length - 1].type)
                return E;
              "start" === _ && e.match(/^.*[\{\(\[]\s*$/) && (E += l);
              return E;
            }),
            (this.checkOutdent = function (_, e, l) {
              return this.$outdent.checkOutdent(e, l);
            }),
            (this.autoOutdent = function (_, e, l) {
              this.$outdent.autoOutdent(e, l);
            }),
  ...
  ace.require(["ace/mode/lsl"], function (_) {
